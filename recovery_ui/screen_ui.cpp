...
/* recovery dark:  #7C4DFF
   ...
   fastbootd dark: #004C99
   fastboot light: #99CCFF */
void ScreenRecoveryUI::SetColor(UIElement e) const {
  switch (e) {
    case UIElement::BATTERY_LOW:
      if (fastbootd_logo_enabled_)
        gr_color(0x99, 0xcc, 0xff, 255);
      else
...
void ScreenRecoveryUI::draw_menu_and_text_buffer_locked(
  ...
  if (menu_) {
    auto& logo = fastbootd_logo_enabled_ ? fastbootd_logo_ : alpha_logo_;
...
void ScreenRecoveryUI::draw_battery_capacity_locked() {
  int x;
  int y = margin_height_ + gr_get_height(alpha_logo_.get());
...
bool ScreenRecoveryUI::Init(const std::string& locale) {
  ...
  if (android::base::GetBoolProperty("ro.boot.dynamic_partitions", false) ||
    ...
    alpha_logo_ = LoadBitmap("logo_image_switch");
    fastbootd_logo_ = LoadBitmap("fastbootd");
  } else {
    alpha_logo_ = LoadBitmap("logo_image");
  }
...
int ScreenRecoveryUI::SelectMenu(const Point& p) {
  ...
  if (menu_) {
    if (!menu_->IsMain()) {
      // Back arrow hitbox
      const static int logo_width = gr_get_width(alpha_logo_.get());
      const static int logo_height = gr_get_height(alpha_logo_.get());
